      ...
      List(selection: $selection) {
        Section("Patterns") {
          ForEach(document.patterns) { rule in
            GrammarOutlineRow(rule: rule, actions: actions)
          }
        }
        if !document.repository.isEmpty {
          Section("Repository") {
            ForEach(document.repository) { entry in
              GrammarOutlineRow(rule: entry.rule, label: entry.name, actions: actions)
            }
          }
        }
  ...
  private var actions: GrammarOutlineActions {
    GrammarOutlineActions(
      add: { shape, placement in
        ...
        switch placement {
        case .after(let id): document.insert(rule, after: id)
        case .inside(let id): document.insert(rule, into: id)
        case .repository: _ = document.addRepositoryRule(named: "new-rule")
        }
