...
/// The moments the application makes a sound.
/// Each played with one of the Mac's own interface sounds rather than a file of its own.
@objc public enum OakSoundIdentifier: Int {
  ...
  /// The file under the system's private interface sound folder.
  /// Those files have lived there since macOS 10.7 and still do on 26.
...
/// Plays an interface sound, loading each once and keeping it.
/// A file that is not there plays as silence, never as an error:
/// the folder is Apple's, not an API, and going quiet is the right failure.
@objc(OakSound)
@MainActor
