...
namespace bundles
{
	...
	bool item_t::initialize (plist::dictionary_t const& plist)
	{
		...
		for(auto const& pair : plist)
		{
			...
			{
				if(std::string const* str = plist::get_if<std::string>(&pair.second))
					_scope_selector = *str;
			}
			...
			{
				if(plist::array_t const* array = plist::get_if<plist::array_t>(&pair.second))
				{
					for(auto const& any : *array)
					{
						if(std::string const* str = plist::get_if<std::string>(&any))
							_fields.emplace(pair.first, *str);
					}
				}
			}
			else if(pair.first == kFieldSettingName)
			{
				...
				else if(plist::array_t const* array = plist::get_if<plist::array_t>(&pair.second))
				{
			...
			else if(pair.first == kFieldRequiredItems && plist::get_if<plist::array_t>(&pair.second))
			{
				for(auto const& dict : plist::get_ref<plist::array_t>(pair.second))
				{
					std::string name;
					oak::uuid_t uuid;
	...
	plist::dictionary_t erase_false_values (plist::dictionary_t const& plist)
	{
		...
		for(auto const& pair : plist)
		{
			if(!plist::get_if<bool>(&pair.second) || plist::get_ref<bool>(pair.second))
