...
namespace ng
{
	...
	std::string character_class (buffer_api_t const& buffer, size_t index)
	{
		...
		if(match)
			return plist::get_ref<std::string>(value);
		...
		else
		{
			...
			if(match && plist::get_ref<std::string>(value).find(buffer[index]) != std::string::npos)
				return kCharacterClassWord;
	...
	static std::vector<std::pair<pattern_t, pattern_t>> character_pairs (scope::context_t const& scope, std::string const& key)
	{
		...
		if(plist::array_t const* array = plist::get_if<plist::array_t>(&value))
		{
			for(auto const& pair : *array)
			{
				...
				{
					std::string const* a1 = value->size() == 2 ? plist::get_if<std::string>(&(*value)[0]) : nullptr;
					std::string const* a2 = value->size() == 2 ? plist::get_if<std::string>(&(*value)[1]) : nullptr;
					if(a1 && a2 && *a1 != *a2)
