...
static void delta_plist_helper (plist::dictionary_t const& oldDict, plist::dictionary_t const& newDict, plist::dictionary_t& changed, plist::array_t& deleted, std::vector<std::string> path = std::vector<std::string>())
{
	...
	for(auto const& newValue : newDict)
	{
		...
		if(plist::get_if<plist::dictionary_t>(&newValue.second) && oldValue != oldDict.end() && plist::get_if<plist::dictionary_t>(&oldValue->second))
		{
			delta_plist_helper(plist::get_ref<plist::dictionary_t>(oldValue->second), plist::get_ref<plist::dictionary_t>(newValue.second), changed, deleted, newPath);
		}
...
static void erase_key_path (plist::dictionary_t& plist, std::string const& keyPath)
{
	...
	for(auto key = v.begin(); key != v.end() && current; )
	{
		...
		if(++key == v.end())
			current->erase(it);
		else if(current = plist::get_if<plist::dictionary_t>(&it->second))
			continue;
		else if(plist::array_t* array = plist::get_if<plist::array_t>(&it->second))
		{
			for(auto it = array->begin(); it != array->end(); ++it)
			{
				if(std::string* str = plist::get_if<std::string>(&(*it)))
				{
					if(*str == *key)
					{
						array->erase(it);
...
static void update_key_path (plist::dictionary_t& plist, std::string const& keyPath, plist::any_t const& value)
{
	...
	for(auto key = v.begin(); key != v.end(); )
	{
		...
		{
			if(current = plist::get_if<plist::dictionary_t>(&it->second))
				continue;
			else if(plist::array_t* array = plist::get_if<plist::array_t>(&it->second))
				array->push_back(*key);
			else
	...
	dictionary_t merge_delta (std::vector<dictionary_t> const& plists)
	{
		...
		riterate(it, plists)
		{
			...
			if(plist.find("isDelta") != plist.end())
			{
				...
				if(array_t const* deleted = deletedIt != plist.end() ? plist::get_if<array_t>(&deletedIt->second) : nullptr)
				{
					for(auto item : *deleted)
					{
						if(std::string const* str = plist::get_if<std::string>(&item))
							erase_key_path(res, *str);
					}
				}

				auto changedIt = plist.find("changed");
				if(dictionary_t const* changed = changedIt != plist.end() ? plist::get_if<dictionary_t>(&changedIt->second) : nullptr)
				{
