...
namespace plist
{
	...
	{
		std::vector<any_t>& ref = get_ref< std::vector<any_t> >(res = std::vector<any_t>());
	...
	dictionary_t convert (CFPropertyListRef plist)
	{
		if(plist && CFGetTypeID(plist) == CFDictionaryGetTypeID())
		{
			...
			return get_ref<dictionary_t>(res);
		}
		return dictionary_t();
	}
	...
	{
		dictionary_t const* dict = get_if<dictionary_t>(&plist);
		if(!dict)
			return false;
	...
	{
		bool flag;
		return !item.valueless_by_exception() && std::visit(convert_to_helper_t<bool>(flag), item) && flag;
	}
