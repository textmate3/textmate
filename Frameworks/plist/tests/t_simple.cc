...
static int32_t parse_int (std::string const& src)
{
	...
	if(int32_t const* res = plist::get_if<int32_t>(&plist))
		return *res;
	return 0;
}
...
{
	plist::any_t plist = plist::parse_ascii(src);
	if(std::string const* res = plist::get_if<std::string>(&plist))
		return *res;
	return NULL_STR;
}
...
{
	plist::any_t plist = plist::parse_ascii(src);
	return plist::get_if< std::vector<plist::any_t> >(&plist);
}

static std::vector<plist::any_t> parse_array (std::string const& src)
{
	plist::any_t plist = plist::parse_ascii(src);
	if(std::vector<plist::any_t> const* res = plist::get_if< std::vector<plist::any_t> >(&plist))
		return *res;
...
{
	plist::any_t plist = plist::parse_ascii(src);
	return plist::get_if< std::map<std::string, plist::any_t> >(&plist);
}

static std::map<std::string, plist::any_t> parse_dict (std::string const& src)
{
	plist::any_t plist = plist::parse_ascii(src);
	if(std::map<std::string, plist::any_t> const* res = plist::get_if< std::map<std::string, plist::any_t> >(&plist))
		return *res;
...
void test_bool ()
{
	...
	OAK_ASSERT(plist::get_if<bool>(&truePlist));
	OAK_ASSERT_EQ(plist::get_ref<bool>(truePlist), true);

	plist::any_t falsePlist = plist::parse_ascii(":false");
	OAK_ASSERT(plist::get_if<bool>(&falsePlist));
	OAK_ASSERT_EQ(plist::get_ref<bool>(falsePlist), false);

	plist::any_t mixedPlist = plist::parse_ascii("( :true, :false )");
	OAK_ASSERT(plist::get_if< std::vector<plist::any_t> >(&mixedPlist));
	std::vector<plist::any_t> const& v = plist::get_ref< std::vector<plist::any_t> >(mixedPlist);
	OAK_ASSERT_EQ(plist::get_ref<bool>(v[0]), true);
	OAK_ASSERT_EQ(plist::get_ref<bool>(v[1]), false);

	// plist::any_t badPlist = plist::parse_ascii(":bad");
	// OAK_ASSERT_EQ(plist::get_if<bool>(&badPlist), (bool*)nullptr);
}
