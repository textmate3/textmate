...
namespace ng
{
	...
	ng::ranges_t editor_t::paste (ng::buffer_t& buffer, ng::ranges_t const& selections, snippet_controller_t& snippets, clipboard_t::entry_ptr entry)
	{
		...
		if(selections.size() == 1 && suitable_for_reindent(str))
		{
			...
			if(text::is_blank(leftOfCaret.data(), leftOfCaret.data() + leftOfCaret.size()))
			{
				...
				std::string const* pasteBehavior = plist::get_if<std::string>(&pasteBehaviorValue);
	...
	static std::string find_paired (std::string const& str, scope::context_t const& scope)
	{
		...
		if(plist::array_t const* typingPairsArray = plist::get_if<plist::array_t>(&typingPairs))
		{
			for(auto const& pair : *typingPairsArray)
			{
				if(plist::array_t const* pairArray = plist::get_if<plist::array_t>(&pair))
				{
					if(pairArray->size() == 2)
					{
						std::string const* left  = plist::get_if<std::string>(&(*pairArray)[0]);
						std::string const* right = plist::get_if<std::string>(&(*pairArray)[1]);
