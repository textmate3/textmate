...
// --allowedFileTypes accepts filename extensions and uniform type identifiers in the same array, and
// the documented examples are all extensions.
//
// Identifiers are tried first because typeWithIdentifier: rejects a bare extension like "pdf", while
// typeWithFilenameExtension: would happily turn an identifier into a dynamic type.
static NSArray<UTType*>* ContentTypesFromStrings (NSArray<NSString*>* types)
{
	NSMutableArray<UTType*>* contentTypes = [NSMutableArray array];
	for(NSString* type in types)
	{
		if(UTType* contentType = [UTType typeWithIdentifier:type] ?: [UTType typeWithFilenameExtension:type])
				[contentTypes addObject:contentType];
		else	fprintf(stderr, "no file type matches ‘%s’ in the value for option '--allowedFileTypes'\n", [type UTF8String]);
	}
	return contentTypes;
}
...
- (NSSavePanel*)setupSavePanel:(NSSavePanel*)savePanel usingParameters:(NSDictionary*)args
{
	...
	if(NSString* typesStr = args[@"allowedFileTypes"])
	{
		...
		if(types)
			[savePanel setAllowedContentTypes:ContentTypesFromStrings(types)];
	}
