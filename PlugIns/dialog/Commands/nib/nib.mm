...
/*
Run this from a command inside TextMate to copy the variables $DIALOG needs onto the pasteboard, then
paste it into a terminal. The examples below work from that shell.

env|egrep 'DIALOG|TM_SUPPORT'|perl -pe 's/(.*?)=(.*)/export $1="$2"/'|pbcopy

"$DIALOG" nib --load "$TM_SUPPORT_PATH/../Bundles/latex.tmbundle/Support/nibs/Preferences.nib"
